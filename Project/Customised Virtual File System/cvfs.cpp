...
SUPERBLOCK SUPERBLOCKobj;//GLOBALLY DECLARED OBJECT
PINODE head=NULL;
...
void man(char*name)
{
	...
	{
		printf("Description : Used to list information of file\n");
		printf("Usage :ls \n");
	}
...
int CloseaFileByName(char*name)
{
	int i=0;
...
int main()
{
	...
	while(1)
	{
		...
		else if(count==2)
		{
			...
			{
				ret=CloseaFileByName(command[1]);
				if(ret==-1)
		           printf("ERROR: There is no such file\n");
				continue;
			}
			...
			else if(count==3)
		{
			...
			else if(strcmp(command[0],"read")==0)
			{
				...
				if(ret==-3)
					printf("ERROR:Reached to end of file\n");
				if(ret==-4)
					printf("ERROR:It is not regular file\n");
				if(ret==0)
